...
function wait(){
	$('<img src="/skin1/main/images/wait.gif" />').modal({
		modal:true,
		escClose:false,
		overlayCss:{
			background:'#000'
...
function gameKanJiangDataC(diffTime, actionNo){
	...
	if(S && h==0 && m==6 && s==0){
		playVoice('/skin1/sound/stop-time.wav', 'stop-time-voice');
	}
	if(h==0 && m==0 && s==0){
		loadKjData();
	}else{
	...
	$(".kj-hao").find("li").attr("flag", "move");
		if(ctype=='g1'){
			...
			moveno = window.setInterval(function () {
				$.each($(".kj-hao").find("li"), function (i, n) {
					if ($(this).attr("flag") == "move") {
						//setTimeout("setKjing2("+i+",'<img src=\"/skin1/main/images/lhc/number/"+hao[i]+".gif\" />')",times);
				    }
				})
			}, 40);
		...
		success:function(data, textStatus, xhr){
			if(!data){
				...
				try{
					...
					playVoice('/skin1/sound/kai-jiang.wav', 'kai-jian-voice');
				}catch(err){
					setTimeout(loadKjData, 5000);
				}
			}
...
/* 任三组六算法*/
function combine_r3z6(arr, num) {
	var rsarr=[];
	arr.forEach(function(v1,i1){
		arr.forEach(function(v2,i2){
			if(v1!=v2&&v1<v2){
				arr.forEach(function(v3,i3){
					if(v2!=v3&&v2<v3){
						rsarr.push(v1+''+v2+''+v3);
					}
				});
...
/* 排列算法,第二种*/
function permutation2(arr, num,weizhi){
	...
	for(var i=0;i<temp.length;i++){
		var tempStr=temp[i];
		rsarr.push(tempStr[0]+''+temp[i]);
		rsarr.push(temp[i]+''+tempStr[1]);
	}
	return rsarr;
}
