...
class User extends WebBase{
	...
	public final function logout(){
		...
		if($this->user['uid']){
			...
			echo "<script>alert('您已安全退出，欢迎再次光临暴雪娱乐!');window.location.href='/index.php/user/login'</script>";
	        exit();
		}
	}
	...
	public final function logined(){
		...
		$remember=wjStrFilter($_POST['remember']);
		if($remember == 1){
	...
	public final function r($userxxx){
		if(!$userxxx){
			if($row_links=$this->getRow("select * from {$this->prename}links where 1=1 order by lid asc limit 0,1")){
				$this->display('team/register.php',0,$this->strToHex($this->myxor($row_links['lid'])) );
			}else{
