...
class Score extends WebLoginBase
{
    ...
    public final function swapGood()
    {
        ...
        try {
            ...
            if (!$good = $this->getRow($sql, $para['goodId']))
    ...
    {
        if (!$swapId = intval($swapId))
            throw new Exception('请求出错');
        if (!$swap = $this->getRow("select * from {$this->prename}score_swap where id=$swapId"))
            throw new Exception('请求出错');
        
        if ($swap['uid'] != $this->user['uid'])
    ...
    public final function rotateEvent()
    {
        ...
        try {
            ...
            if (in_array($rid, $money)) {
                ...
                if (!$this->insertRow($this->prename . 'dzp_swap', $para)) {
                    $result['angle'] = 0;
                    $result['prize'] = '内部出错，请重新操作!';
                    return $result;
                } //产生后台记录
    ...
    public final function dbqbed()
    {
        ...
        if (!$this->dbqbsettings['switchWeb'])
            throw new Exception('夺宝奇兵活动已下线，敬请期待!');
        if ($this->time < $dateq || $this->time > $datez)
        ...
        $con = number_format($this->getValue("select sum(beiShu * mode * actionNum) from {$this->prename}bets where actionTime > {$jrdate} and uid={$this->user['uid']} and isDelete=0"), 2);
        if ($con < number_format($this->dbqbsettings['xcoin'], 2))
            throw new Exception('今日消费' . $con . ',不满' . $this->dbqbsettings['xcoin'] . '元，无法参加!');
        
        if ($this->dbqbsettings['num'] <= 0)
            throw new Exception('很遗憾！宝箱已被抢光，请等待下一场！');
    ...
    {
        //print_r($this->dzyhsettings); exit;
    ...
    {
        $ckdate1 = $this->dzyhsettings['ckdate1'] * 24;
        $cklv1   = $this->dzyhsettings['cklv1'];
        $ckdate2 = $this->dzyhsettings['ckdate2'] * 24;
        $cklv2   = $this->dzyhsettings['cklv2'];
        $ckdate3 = $this->dzyhsettings['ckdate3'] * 24;
        $cklv3   = $this->dzyhsettings['cklv3'];
        $ckdate4 = $this->dzyhsettings['ckdate4'] * 24 * 30;
        $cklv4   = $this->dzyhsettings['cklv4'];
        $ckdate5 = $this->dzyhsettings['ckdate5'] * 24 * 30 * 12;
        $cklv5   = $this->dzyhsettings['cklv5'];
        ...
        if ($time < $this->dzyhsettings['ckzdsj']) {
            ...
            $lv = $cklv[0];
            $lx = $data['ck_money'] * $cklv[0] / 100 * $cktime[0] / 24;
        } else if ($time >= $cktime[1] && $time < $cktime[2]) {
            $lv = $cklv[1];
            $lx = $data['ck_money'] * $cklv[1] / 100 * $cktime[1] / 24;
        } else if ($time >= $cktime[2] && $time < $cktime[3]) {
            $lv = $cklv[2];
            $lx = $data['ck_money'] * $cklv[2] / 100 * $cktime[2] / 24;
        } else if ($time >= $cktime[3] && $time < $cktime[4]) {
            $lv = $cklv[3];
            $lx = $data['ck_money'] * $cklv[3] / 100 * $cktime[3] / 24;
        } else if ($time >= $cktime[4]) {
            $lv = $cklv[4];
            $lx = $data['ck_money'] * $cklv[4] / 100 * $cktime[4] / 24;
        }
        $data['lx'] = $lx;
    ...
    public final function dzyhcked()
    {
        ...
        if (!$this->dzyhsettings['switchck'])
            throw new Exception('电子银行存款功能已关闭,详情请联系平台客服！'); //存款开关判断
        ...
        if ($ck_money <= 0)
            throw new Exception('存款不能为负数，请勿抓包！');
        if ($ck_money > $this->user['coin'])
        ...
        if (!$this->insertRow($this->prename . 'dzyh_ck_swap', $para))
            throw new Exception('存款失败！请重试'); //产生存款记录
    ...
    public final function dzyhtked()
    {
        ...
        if (!$this->dzyhsettings['switchtk'])
            throw new Exception('电子银行提款功能已关闭,详情请联系平台客服！'); //提款开关判断
        ...
        if ($this->getValue("select enable from {$this->prename}dzyh_ck_swap where uid={$this->user['uid']} and isdelete=0 and state=0;"))
            throw new Exception('对不起！您的存款已被冻结,详情请联系平台客服！');
        $ckdate1 = $this->dzyhsettings['ckdate1'] * 24;
        $cklv1   = $this->dzyhsettings['cklv1'];
        $ckdate2 = $this->dzyhsettings['ckdate2'] * 24;
        $cklv2   = $this->dzyhsettings['cklv2'];
        ...
        if (!$this->update("update {$this->prename}dzyh_ck_swap set state=1 where uid={$this->user['uid']}"))
            throw new Exception('提款失败！请重试'); //重置提款状态
        if (!$this->insertRow($this->prename . 'dzyh_tk_swap', $para))
            throw new Exception('提款失败！请重试'); //产生提款记录
