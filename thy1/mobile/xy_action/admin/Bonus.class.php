...
class Bonus extends AdminBase
{
    ...
    public final function bonusLogDelete($id)
    {
        ...
        if ($bonusStatus == 1) {
            $this->updateRows($this->prename . 'bonus_log', array(
                'isDelete' => 1
            ), 'id=' . $id);
            echo '操作成功';
        }
    ...
    public final function shareBonusSingle($uid)
    {
        ...
        if (!isset($para['lossAmount']) || empty($para['lossAmount']))
            throw new Exception('参数出错');
        if (!isset($para['bonusAmount']) || empty($para['bonusAmount']))
            throw new Exception('参数出错');
        if (!isset($para['startTime']) || empty($para['startTime']))
            throw new Exception('参数出错');
        if (!isset($para['endTime']) || empty($para['endTime']))
            throw new Exception('参数出错');
        ...
        if ($bonusId)
            throw new Exception('该用户本期分红已经发放');
