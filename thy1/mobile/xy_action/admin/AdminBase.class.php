...
class AdminBase extends Object
{
    ...
    public $settings;
    public $adminLogType = array(1 => '提现处理', 2 => '充值确认', 3 => '管理员充值', 4 => '增加用户', 5 => '修改用户', 6 => '删除用户', 7 => '添加管理员', 8 => '修改管理员密码', 9 => '删除管理员', 10 => '修改系统设置', 11 => '银行设置', 12 => '彩种设置', 13 => '玩法设置', 14 => '等级设置修改', 15 => '兑换订单处理', 16 => '兑换商品操作', 17 => '手动开奖', 18 => '修改订单', 19 => '清除管理员', 20 => '添加支付接口', 21 => '踢会员下线', 22 => '大转盘中奖记录处理', 23 => '修改大转盘设置', 24 => '修改夺宝奇兵配置', 25 => '夺宝记录处理', 26 => '电子银行提款记录删除', 27 => '电子银行存款冻结', 28 => '电子银行存款解冻');
    ...
    public function getUser($uid = null)
    {
        if ($uid === null)
            return $this->user;
        if (is_int($uid))
            return $this->getRow("select * from {$this->prename}sysmember where uid='$uid'");
        if (is_string($uid))
            return $this->getRow("select * from {$this->prename}sysmember where username=?", $uid);
    }
    ...
    {
        if ($user = unserialize($_SESSION[$this->w234FWEF32F]))
            return $user;
