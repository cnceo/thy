...
class Dbqb extends AdminBase
{
    ...
    public final function updateSettings()
    {
        ...
        if (!ctype_digit($para['scoin']))
            throw new Exception('请正确设置账户余额!');
        if (!ctype_digit($para['xcoin']))
            throw new Exception('请正确设置今日消费!');
        if (!ctype_digit($para['num']))
            throw new Exception('请正确设置宝箱数量!');
        if ($para['switchWeb'] != 0 && $para['switchWeb'] != 1)
            throw new Exception('请正确设置夺宝奇兵开关!');
        
        $para['allnum'] = $para['num'];
        if (substr($para['value'], -1) == '*' || substr($para['value'], 0) == '*')
            throw new Exception('金额格式错误，请重新配置!');
