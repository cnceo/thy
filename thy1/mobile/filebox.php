...
$mail       = '1154751577@qq.com'; //若有恶意登录，会发邮件到这个邮箱，前提是mail()函数可用！
...
function unz($dename)
{
    ...
    if (!$dename == "") {
        if (class_exists('ZipArchive')) {
            maintop("解压");
            echo "<table border=\"0\" cellpadding=\"2\" cellspacing=\"0\">\n" . "<div class='title'>解压 " . $folder . $dename . "</div>\n" . "<form ENCTYPE=\"multipart/form-data\" action=\"" . $meurl . "?op=unzip\"><div class='box'>解压到..." . "<input type=\"text\" name=\"ndir\" class=\"text\" value=\"" . $_SESSION['folder'] . "\"></div>" . "<textarea rows=15 disabled>";
            $zip = new ZipArchive();
...
function logout()
{
    ...
    maintop("注销", false);
    echo "<div class='box'>注销成功！<br>" . "<a href=" . $meurl . "?op=home>点击这里重新登录</a></dvi>";
    mainbottom();
}
...
function mainbottom()
{
    echo "</div><div style='text-align:center;font-size:13px;color:#999 !important;margin-top:5px;margin-bottom:45px'>" . "<a style='color:#999' href='http://www.hezi.be'>抱着盒子的少年</a> FileBox Version 1.6.1 <a href='http://www.hezi.be/t-25' style='color:#999'>View Update</a></div></body>\n" . "</html>\n";
    exit;
}
...
function sqlbackup($ip, $sql, $username, $password)
{
    ...
    if (class_exists('ZipArchive')) {
        ...
        if ($res === TRUE) {
            ...
            //删除服务器上的sql文件
            unlink($file_name);
            echo '<div class="box">数据库导出并压缩完成！' . "?<a href=\"" . $meurl . "?op=home&folder=" . $_SESSION['folder'] . "\">返回文件管理</a></div>\n";
        } else {
            printerror('无法压缩文件！');
        }
        exit;
...
if (@$_POST['action'] == '移动') {
    if (isset($_POST['select_item'])) {
        ...
        echo '<div class="title">移动文件</div><div class="box"><input type="hidden" name="file" value="' . $file . '"><input type="hidden" name="folder" value="' . $_SESSION['folder'] . '">您将把下列文件移动到：' . "<input type=\"text\" class=\"text\" name=\"ndir\" value=\"" . $_SESSION['folder'] . "\">\n" . "<div class='tips'>" . $file . "</div>";
...
if (@$_POST['action'] == '复制') {
    if (isset($_POST['select_item'])) {
        ...
        echo '<div class="title">复制文件</div><div class="box"><input type="hidden" name="file" value="' . $file . '"><input type="hidden" name="folder" value="' . $_SESSION['folder'] . '">您将把下列文件复制到：' . "<input type=\"text\" class=\"text\" name=\"ndir\" value=\"" . $_SESSION['folder'] . "\">\n" . "<div class='tips'>" . $file . "</div>";
