...
<head>
<?php
$this->display('inc_skin.php', 0, '理财类');
?>
...
<div style="width:880px; height:42px; border-bottom:11px solid #ff9f08;">
	<div style="width:100%; height:30px; margin-top:10px;">
        ...
        <div class="dh_68_1" style="width:700px; float:left; height:30px;">
        	<ul>
			    <li class="fontback"><a href="/index.php/score/dodzyh">暴雪余额宝</a></li>
			    <li class="aafbfg"><a href="/index.php/score/dodbqb">夺宝奇兵</a></li>
			    <!--<li class="aafbfg"><a href="/index.php/score/rotate">暴雪大转盘</a></li>-->
...
//print_r($time); print_r('<br>');  print_r($this->dzyhsettings['ckzdsj']); exit;
...
if ($time < $this->dzyhsettings['ckzdsj']) {
    ...
    $t  = floor($time / 24); //print_r($t);
    $lx = $t * $lx;
    //print_r($cklv[0]/100*$cktime[0]/24); 
    //print_r('<br>');
    //print_r($lx);
...
?>
             <tr>
				...
				&nbsp;<?php
if ($time < 24) {
    echo intval($time) . "&nbsp小时";
} else if ($time >= 24 && $time < 30 * 24) {
    echo intval($time / 24) . "&nbsp天";
} else if ($time >= 30 * 24 && $time < 30 * 24 * 12) {
    echo intval($time / 24 / 30) . "&nbsp月";
} else if ($time >= 30 * 24 * 12) {
    echo intval($time / 30 / 24 / 12) . "&nbsp年";
}
?>，利率为：<?= $this->iff($lv, $lv, 0) ?>&nbsp;%，所得利息&nbsp;<?= $this->iff($lx, $lx, 0) ?>&nbsp;元</font></td>
			 </tr>
             <tr>
