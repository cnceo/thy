...
<?php
$this->display('inc_skin.php', 0, '会员中心 - 提现记录');
?>
...
if ($_GET['fromTime'] && $_GET['endTime']) {
    ...
    if ($GLOBALS['fromTime'] && $GLOBALS['toTime'])
        $sql .= ' and actionTime between ' . $GLOBALS['fromTime'] . ' and ' . $GLOBALS['toTime'] . ' ';
}
...
?>
            <tr style="100%">
                <td style="10%"><?= $var['amount'] ?></td>
                <td style="10%"><?= date('m-d H:i:s', $var['actionTime']) ?></td>
                <td style="10%"><?= $var['bankName'] ?></td>
                <td style="10%"><?= preg_replace('/^.*(.{4})$/', "$1", $var['account']) ?></td>
                <td style="10%">
                <?php
        if ($var['state'] == 3) {
