...
foreach ($times['data'] as $var) {
    if ($para['actionNo']) {
        ...
        $var        = array(
            'actionNo' => '--',
            'actionTime' => date('Y-m-d H:i:s', $date)
        );
        $dateString = '';
    } else {
        if ($this->type == 1) {
...
?>
                ...
                <td>
                <a href="/admin778899.php/data/back/<?= $this->type ?>/<?= $var['actionNo'] ?>/<?= $dateString . $var['actionTime'] ?>"  target="modal" width="340" title="对未开奖期号进行投注退款"  modal="true" button="确定:dataAddCode|取消:defaultCloseModal" style="color:#F00;">退款</a>
                </td>
			</tr>
			<?php
}
