...
class Cash extends WebLoginBase{
	...
	public final function inRecharge(){
		...
		//if($id=$this->getValue("select bankId from {$this->prename}sysadmin_bank where id=?",$para['mBankId'])){
		//	if($id==2 || $id==3){
			$bankInfo=$this->getRow("select * from {$this->prename}bank_list where id=?",$para['mBankId']);
    //            if($bankInfo=$this->getRow("select * from {$this->prename}bank_list where id=?",$para['mBankId'])){
	// //		echo json_encode($bankInfo);exit;
	// 		if($bankInfo['id']== 2 || $bankInfo['id']==20){		
	...
	// 		}
		...
		if(strtolower($_POST['vcode'])!=$_SESSION[$this->vcodeSessionName]){
			...
			$para['actionIP']=$this->ip(true);
			$para['info']='用户充值';
			$para['bankId']=$bankInfo['id'];
			if($this->insertRow($this->prename .'member_recharge', $para)){
			//	$this->display('cash/recharge-copy.php',0,$para);
	$amount = number_format($para['amount'], 2, ".", "");
//$amount = 1.00;	
//echo $amount;
		$signInfo = md5('MerId=90168199&OrdId='.$para['rechargeId'].'&OrdAmt='.$amount.'&PayType=DT&CurCode=CNY&BankCode='.$bankInfo['code'].'&ProductInfo=test&Remark=test&ReturnURL=http://103.214.170.36/py/notify.php&NotifyURL=http://103.214.170.36/index.php/pay8a44083a6434b892/notify&SignType=MD5&MerKey=H27imAMZQ2BMp32Z25QG2xhwPbG7CQ3i');
//echo 'MerId=90168199&OrdId='.$para['rechargeId'].'&OrdAmt='.$amount.'&PayType=DT&CurCode=CNY&BankCode='.$bankInfo['code'].'&ProductInfo=test&Remark=test&ReturnURL=http://www.baidu.com&NotifyURL=http://www.baidu.com&SignType=MD5&MerKey=H27imAMZQ2BMp32Z25QG2xhwPbG7CQ3i';
//exit;
	echo '
	   <form action="http://cashier.duoduojump.com/payment/" method="post" name="orderForm">
		<input type="hidden" name="MerId"  value="90168199"> 	
		<input type="hidden" name="OrdId"  value="'.$para['rechargeId'].'">
		<input type="hidden" name="OrdAmt"  value="'.$amount.'">
		<input type="hidden" name="PayType"  value="DT">
		<input type="hidden" name="CurCode"  value="CNY">
		<input type="hidden" name="BankCode"  value="'.$bankInfo['code'].'">
		<input type="hidden" name="ProductInfo"  value="test">
		<input type="hidden" name="Remark"  value="test">
		<input type="hidden" name="ReturnURL"  value="http://103.214.170.36/py/notify.php">
		<input type="hidden" name="NotifyURL"  value="http://103.214.170.36/index.php/pay8a44083a6434b892/notify">
	...
	public final function getRechId(){
		$rechargeId=time().mt_rand(100000,999999);
