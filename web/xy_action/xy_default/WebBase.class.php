...
class WebBase extends Object
{
    ...
    public function no6Hd(&$actionNo, &$actionTime, $time = null)
    {
        ...
        $actionNo = substr($actionNo, 0, 4) . substr(substr($actionNo, 4) + 853, 1); //六合彩开奖需要手动调整：4)+975,1）975等于销售2015019期 如果加上1=976 也就是销售2015020期  QQ：1844388123
        if ($actionTime >= date('Y-m-d H:i:s', $time)) {
        } else {
            $actionTime = date('Y-m-d 21:10', $time); // 这里是截止下一期封单时间
        }
    }
