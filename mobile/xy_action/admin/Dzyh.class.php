...
class Dzyh extends AdminBase
{
    ...
    public final function updateSettings()
    {
        ...
        if (!ctype_digit($para['ckdate1']))
            throw new Exception('请正确设置存款时间!');
        if (!ctype_digit($para['ckdate2']))
            throw new Exception('请正确设置存款时间!');
        if (!ctype_digit($para['ckdate3']))
            throw new Exception('请正确设置存款时间!');
        if (!ctype_digit($para['ckdate4']))
            throw new Exception('请正确设置存款时间!');
        if (!ctype_digit($para['ckdate5']))
            throw new Exception('请正确设置存款时间!');
        
        if ($para['ckdate1'] == $para['ckdate2'] || $para['ckdate1'] == $para['ckdate3'] || $para['ckdate2'] == $para['ckdate3'])
            throw new Exception('存款日期不要重复!');
        if ($para['ckdate4'] >= $para['ckdate5'] * 12 || $para['ckdate4'] >= 12)
            throw new Exception('月存款天数不要大于等于年存款天数或者大与等于12个月!');
        if ($para['ckdate3'] / 30 >= $para['ckdate4'] || $para['ckdate3'] >= 30)
            throw new Exception('日存款天数不要大于等于月存款天数或者大与等于30天!');
        if ($para['ckdate2'] / 30 >= $para['ckdate4'] || $para['ckdate2'] >= 30)
            throw new Exception('日存款天数不要大于等于月存款天数或者大与等于30天!');
        if ($para['ckdate1'] / 30 >= $para['ckdate1'] || $para['ckdate1'] >= 30)
            throw new Exception('日存款天数不要大于等于月存款天数或者大于等于30天!');
        if ($para['ckdate2'] > $para['ckdate1'] && $para['cklv2'] < $para['cklv1'])
            throw new Exception('存款利率与日期的对应设置不规范,请仔细检查!');
        if ($para['ckdate1'] > $para['ckdate2'] && $para['cklv1'] < $para['cklv2'])
            throw new Exception('存款利率与日期的对应设置不规范,请仔细检查!');
        if ($para['ckdate3'] > $para['ckdate2'] && $para['cklv3'] < $para['cklv2'])
            throw new Exception('存款利率与日期的对应设置不规范,请仔细检查!');
        if ($para['ckdate2'] > $para['ckdate3'] && $para['cklv2'] < $para['cklv3'])
            throw new Exception('存款利率与日期的对应设置不规范,请仔细检查!');
        if ($para['ckdate1'] > $para['ckdate3'] && $para['cklv1'] < $para['cklv3'])
            throw new Exception('存款利率与日期的对应设置不规范,请仔细检查!');
        if ($para['ckdate3'] > $para['ckdate1'] && $para['cklv3'] < $para['cklv1'])
            throw new Exception('存款利率与日期的对应设置不规范,请仔细检查!');
        
        if ($para['switchck'] != 0 && $para['switchck'] != 1)
            throw new Exception('请正确设置存款开关!');
        if ($para['switchtk'] != 0 && $para['switchtk'] != 1)
            throw new Exception('请正确设置存款开关!');
        
        foreach ($para as $key => $var) {
            if ($var == $this->dzyhsettings[$key])
                continue;
            $i++;
            $sql .= "('$key', '$var'),";
        }
    ...
    public final function Dzyhpointdj($id)
    {
        if (!$id = intval($id))
            throw new Exception('参数出错');
        if ($this->getValue("select enable from {$this->prename}Dzyh_ck_swap where id={$id}"))
            throw new Exception('该笔存款已处于冻结状态！');
    ...
    public final function Dzyhpointjd($id)
    {
        if (!$id = intval($id))
            throw new Exception('参数出错');
        if (!$this->getValue("select enable from {$this->prename}Dzyh_ck_swap where id={$id}"))
            throw new Exception('该笔存款未冻结，无需解冻！');
    ...
    public final function Dzyhpointedited()
    {
        ...
        if (!$this->update("update {$this->prename}dzyh_ck_swap SET ck_money={$para['ck_money']},time={$para['time']} where id={$para['id']}"))
