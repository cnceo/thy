...
class box extends WebLoginBase
{
    ...
    public final function dowrite()
    {
        ...
        if ($touser == 'parent') {
            ...
            if (!$parentid = $this->getValue($sql, $this->user['uid']))
                throw new Exception('您无上级代理！');
            ...
            foreach ($arr as $key => $var) {
                if ($this->getValue($sql, $arr[$key]) != $this->user['uid'])
                    throw new Exception('某些会员不是您的直属下级！');
    ...
    public final function answer($mid)
    {
        ...
        if ($this->getValue($sql2, $mid) != $this->user['uid'])
            throw new Exception('这不是您的消息,无法回复！');
    ...
    public final function deleteAll($id)
    {
        ...
        $sql2 = "select to_uid from {$this->prename}message_receiver where mid=? and to_username='{$this->user['username']}'";
        foreach ($arr as $key => $var) {
            if ($this->getValue($sql2, $arr[$key]) != $this->user['uid'])
                throw new Exception('这不是您的消息,无法删除！');
    ...
    public final function detail($mid)
    {
        ...
        if ($this->getValue($sql3, $mid) != $this->user['uid'])
            throw new Exception('这不是您的消息！');
    ...
    public final function senddetail($mid)
    {
        ...
        $sql2 = "select from_uid from {$this->prename}message_sender where mid=?";
        if ($this->getValue($sql2, $mid) != $this->user['uid'])
            throw new Exception('这不是您发送过的消息！');
