...
class Team extends WebLoginBase
{
    ...
    public final function insertLink()
    {
        ...
        if ($update['fanDian'] < 0)
            throw new Exception('返点不能小于0');
        if ($update['fanDian'] > $this->iff($this->user['fanDian'] - $this->settings['fanDianDiff'] < 0, 0, $this->user['fanDian'] - $this->settings['fanDianDiff']))
            throw new Exception('返点不能大于' . $this->iff($this->user['fanDian'] - $this->settings['fanDianDiff'] < 0, 0, $this->user['fanDian'] - $this->settings['fanDianDiff']));
        if ($update['type'] != 0 && $update['type'] != 1)
            throw new Exception('类型出错，请重新操作');
        if ($update['uid'] != $this->user['uid'])
            throw new Exception('只能增加自己的推广链接!');
        
        // 查检返点设置
        if ($update['fanDian']) {
            $this->getSystemSettings();
            if ($update['fanDian'] % $this->settings['fanDianDiff'])
                throw new Exception(sprintf('返点只能是%.1f%的倍数', $this->settings['fanDianDiff']));
        } else {
            $update['fanDian'] = 0.0;
        }
    ...
    public final function linkUpdateed()
    {
        ...
        if ($uid = $this->getvalue("select uid from {$this->prename}links where lid=?", $lid)) {
            if ($uid != $this->user['uid'])
                throw new Exception('只能修改自己的推广链接!');
        } else {
    ...
    public final function linkDeleteed()
    {
        ...
        if ($uid = $this->getvalue("select uid from {$this->prename}links where lid=?", $lid)) {
            if ($uid != $this->user['uid'])
                throw new Exception('只能删除自己的推广链接!');
        } else {
            throw new Exception('此注册链接不存在');
        }
    ...
    public final function insertMember()
    {
        ...
        if ($update['fanDian'] > $this->iff($this->user['fanDian'] - $this->settings['fanDianDiff'] <= 0, 0, $this->user['fanDian'] - $this->settings['fanDianDiff']))
            throw new Exception('返点不能大于' . $this->iff($this->user['fanDian'] - $this->settings['fanDianDiff'] < 0, 0, $this->user['fanDian'] - $this->settings['fanDianDiff']));
        if (!$update['username'])
            throw new Exception('用户名不能为空，请重新操作');
        if ($update['type'] != 0 && $update['type'] != 1)
            throw new Exception('类型出错，请重新操作');
        ...
        if ($userlen < 4 || $userlen > 16)
            throw new Exception('用户名长度不正确,请重新输入');
        if ($passlen < 6)
            throw new Exception('密码至少六位,请重新输入');
        if ($qqlen < 4 || $qqlen > 13)
        ...
        if ($update['fanDian']) {
            ...
            if ($count && $count['registerCount'] >= $count['userCount'])
                throw new Exception(sprintf('对不起返点为%.1f的下级人数已经达到上限', $update['fanDian']));
        } else {
            $update['fanDian'] = 0.0;
        }
        ...
        try {
            ...
            if ($this->getValue($sql, $update['username']))
                throw new Exception('用户“' . $update['username'] . '”已经存在');
    ...
    public final function userUpdateed()
    {
        ...
        if ($parentId != $this->user['uid'])
            throw new Exception('此会员不是你的直属下线，无法修改');
    ...
    public final function userUpdateed2()
    {
        ...
        try {
            ...
            if ($userData2['parentId'] != $uid2)
                throw new Exception('此会员不是的你的直属会员，请重新选择！');
            if (!$userData2['enable'])
                throw new Exception('此会员已被冻结，无法转移！');
            if ($userData['coin'] <= 0)
                throw new Exception('余额不足，请先充值！');
            if ($userData['coin'] < $para['coin'])
                throw new Exception('可用余额小于充值金额，请先充值！');
            ...
            if ($bank['bankId']) {
                ...
                try {
                    ...
                    if (!$this->insertRow($this->prename . 'member_cash', $para))
                        throw new Exception('领取分红请求出错');
                    if (!$this->updateRows($this->prename . 'bonus_log', array(
                        'bonusStatus' => 1
                    ), 'id=' . $lastBonus['id']))
                        throw new Exception('领取分红请求出错');
                    $id = $this->lastInsertId();
