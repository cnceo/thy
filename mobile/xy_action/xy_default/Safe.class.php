...
class Safe extends WebLoginBase
{
    ...
    public final function setPasswd()
    {
        ...
        if (strlen($opwd) < 6)
            throw new Exception('原密码至少6位');
        if (!$npwd = $_POST['newpassword'])
    ...
    public final function setCoinPwd()
    {
        ...
        if (!$pwd['coinPassword']) {
            ...
            if ($npwd == $pwd['password'])
                throw new Exception('资金密码与登录密码不能一样');
            $tishi = '资金密码设置成功';
        } else {
    ...
    public final function setCoinPwd2()
    {
        $opwd = $_POST['oldpassword'];
        if (!$opwd)
            throw new Exception('原资金密码不能为空');
        if (strlen($opwd) < 6)
            throw new Exception('原资金密码至少6位');
        if (!$npwd = $_POST['newpassword'])
            throw new Exception('资金密码不能为空');
        if (strlen($npwd) < 6)
            throw new Exception('资金密码至少6位');
    ...
    public final function setCBAccount()
    {
        ...
        if (!isset($update['countname']))
            throw new Exception('请填写开户行!');
        if (!isset($update['username']))
            throw new Exception('请填写账户名!');
        if (!isset($update['bankId']))
            throw new Exception('请选择银行类型!');
        ...
        if (($x != $y && $x % $y == 0) == FALSE)
            throw new Exception('开户行必须为汉字');
        if (($a != $b && $a % $b == 0) == FALSE)
            throw new Exception('用户名必须为汉字');
        unset($x);
        unset($y);
        unset($a);
        unset($b);
        ...
        // 更新用户信息缓存
        $this->freshSession();
        if (md5($update['coinPassword']) != $this->user['coinPassword'])
        ...
        //检查银行账号唯一
        if ($account = $this->getValue("select account FROM {$this->prename}member_bank where account=? LIMIT 1", $update['account']))
            throw new Exception('该' . $account . '银行账号已经使用');
        //检查账户名唯一
        if ($account = $this->getValue("select username FROM {$this->prename}member_bank where account=? LIMIT 1", $update['username']))
            throw new Exception('该' . $username . '账户名已经使用');
        ...
        if ($bank = $this->getRow("select editEnable from {$this->prename}member_bank where uid=? LIMIT 1", $this->user['uid'])) {
            ...
            if ($bank['editEnable'] != 1)
                throw new Exception('银行信息绑定后不能随便更改，如需更改，请联系在线客服');
    ...
    {
        if (!$_POST)
            throw new Exception('提交参数出错');
        
        //过滤未知字段
        $update['care'] = wjStrFilter(@addslashes($_POST['care']));
        
        //问候语长度限制
        $len = mb_strlen($update['care'], 'utf8');
        if ($len > 10)
            throw new Exception('登陆问候语过长，请重新输入');
        if ($len = 0)
            throw new Exception('登陆问候语不能为空，请重新输入');
    ...
    public final function nickname()
    {
        ...
        //过滤未知字段
        $update['nickname'] = wjStrFilter(@addslashes($_POST['nickname']));
        
        $len = mb_strlen($update['nickname'], 'utf8');
        if ($len > 8)
            throw new Exception('昵称过长，请重新输入');
        if ($len = 0)
