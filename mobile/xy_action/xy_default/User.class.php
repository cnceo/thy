...
class User extends WebBase{
	...
	public final function logined(){
		...
		$vcode=$_POST['vcode'];
        if(!ctype_alnum($username)) throw new Exception('用户名不正确,请重新输入');
		...
		if($vcode!=$_SESSION[$this->vcodeSessionName]){
			throw new Exception('验证码错误，请换个姿势输入');
		}
		...
		if(!$user['enable']){
			throw new Exception('您的帐号被冻结，请联系客服。');
		}
		$session=array(
			'uid'=>$user['uid'],
    ...
    public final function reg(){
		if(!$_POST)  throw new Exception('开户失败，请重新操作');
		...
		if(!ctype_alnum($user)) throw new Exception('用户名格式不正确');
		if(!ctype_digit($qq)) throw new Exception('QQ格式不正确');
