...
$sql = "select u.username, u.coin, u.uid, u.parentId, sum(b.mode * b.beiShu * b.actionNum) betAmount, sum(b.bonus) zjAmount, (select sum(c.amount) from xy_member_cash c where c.state=0 and c.`uid`=u.`uid` $cashTimeWhere) cashAmount,(select sum(r.amount) from xy_member_recharge r where r.state in(1,2,9) and r.`uid`=u.`uid` $rechargeTimeWhere) rechargeAmount, (select sum(l.coin) from xy_coin_log l where l.liqType in(50,51,52,53,54,56,150,140,130,120,110) and l.`uid`=u.`uid` $brokerageTimeWhere) brokerageAmount from xy_members u left join xy_bets b on b.isDelete=0 and u.uid=b.uid $betTimeWhere where 1 $userWhere";
...
$sql2                   = "select sum(b.mode * b.beiShu * b.actionNum) betAmount, sum(b.bonus) zjAmount from xy_members u left join xy_bets b on b.isDelete=0 and u.uid=b.uid $betTimeWhere $userWhere3";
...
if ($list['data'])
    foreach ($list['data'] as $var) {
        if ($var['username'] != '没有下级了') {
            ...
            $pId                  = $var['uid'];
            $var['teamwin']       = $this->getValue("select sum(l.coin) fandianAll from {$this->prename}coin_log l, xy_members u where l.liqType in(2,3) and l.uid=u.uid and concat(',', u.parents, ',') like '%,$pId,%' $fanDiaTimeWhere") + $this->getValue("select sum(b.bonus-b.mode * b.beiShu * b.actionNum) betZjAmount from xy_members u ,xy_bets b where b.isDelete=0 and u.uid=b.uid and concat(',', u.parents, ',') like '%,$pId,%' $betTimeWhere");
        }
