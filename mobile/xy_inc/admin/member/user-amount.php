<?php
$sql = "select u.username, u.coin, u.uid, u.parentId, sum(b.mode * b.beiShu * b.actionNum) betAmount, sum(b.bonus) zjAmount,(select sum(c.amount) from xy_member_cash c where c.`uid`=u.`uid` and c.state=0) cashAmount,(select sum(r.amount) from xy_member_recharge r where r.`uid`=u.`uid` and r.state in(1,2,9)) rechargeAmount, (select sum(l.coin) from xy_coin_log l where l.`uid`=u.`uid` and l.liqType in(50,51,52,53)) brokerageAmount from xy_members u left join xy_bets b on u.uid=b.uid and b.isDelete=0 where u.uid=?";
