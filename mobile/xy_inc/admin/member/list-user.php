<?php
$sql = "select u.*, sum(b.mode * b.beiShu * b.actionNum) betAmount, sum(b.bonus) zjAmount, (select sum(c.amount) from xy_member_cash c where c.`uid`=u.`uid` and c.state=0 ) cashAmount,(select sum(r.amount) from xy_member_recharge r where r.`uid`=u.`uid` and r.state in(1,2,9) ) rechargeAmount from xy_members u left join xy_bets b on u.uid=b.uid and b.isDelete=0  where  u.admin=0";
...
?></td>
        ...
        <td><?= $this->iff($login['loginTime'], date('m-d H:i', $login['loginTime']), '--') ?><?php //=$var['updateTime']
?></td> 
