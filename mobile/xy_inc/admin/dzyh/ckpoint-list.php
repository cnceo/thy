...
if ($data['data'])
    foreach ($data['data'] as $var) {
        ...
        if ($time < $this->dzyhsettings['ckzdsj']) {
            ...
            $lv = $cklv[0];
            $lx = $var['ck_money'] * $cklv[0] / 100 * $cktime[0] / 24;
        } else if ($time >= $cktime[1] && $time < $cktime[2]) {
            $lv = $cklv[1];
            $lx = $var['ck_money'] * $cklv[1] / 100 * $cktime[1] / 24;
        } else if ($time >= $cktime[2] && $time < $cktime[3]) {
            $lv = $cklv[2];
            $lx = $var['ck_money'] * $cklv[2] / 100 * $cktime[2] / 24;
        } else if ($time >= $cktime[3] && $time < $cktime[4]) {
            $lv = $cklv[3];
            $lx = $var['ck_money'] * $cklv[3] / 100 * $cktime[3] / 24;
        } else if ($time >= $cktime[4]) {
            $lv = $cklv[4];
            $lx = $var['ck_money'] * $cklv[4] / 100 * $cktime[4] / 24;
        }
?>
		<tr> 
			<td><?= $var['id'] ?></td>
