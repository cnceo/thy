...
if ($_GET['fromTime'] && $_GET['toTime']) {
    ...
    if ($GLOBALS['fromTime'] && $GLOBALS['toTime'])
        $sql .= ' and c.actionTime between ' . $GLOBALS['fromTime'] . ' and ' . $GLOBALS['toTime'] . ' ';
}
...
?>
		<tr width="100%">
			<td width="15%"><?= $var['amount'] ?></td>
			<td width="15%"><?= $var['username'] ?></td>
			<td width="15%"><?= date('m-d H:i:s', $var['actionTime']) ?></td>
			<td width="15%"><?= $var['bankName'] ?></td>
			<td width="15%"><?= preg_replace('/^.*(.{4})$/', '\1', $var['account']) ?></td>
			<td width="15%">
			<?php
