...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr class="top" style="width:100%">
			...
			<td style="width:10%">在线</td>
			
			<td style="width:10%">注册时间</td>
			<td width="17%" style="width:10%">操作</td>
		</tr>
	<?php
if ($data['data'])
...
?>
		<tr style="width:100%">
			<td style="width:10%"><?= $var['username'] ?></td>
        
            <td style="width:10%"><?= $var['fanDian'] ?>%</td>
			<td style="width:10%"><?= $var['coin'] ?></td>
			
			<td style="width:10%"><?= $this->iff($var['enable'], '正常', '冻结') ?></td>
            <td style="width:10%"><?= $this->iff($login['isOnLine'] && ceil(strtotime(date('Y-m-d H:i:s', time())) - strtotime(date('Y-m-d H:i:s', $login['accessTime']))) < $GLOBALS['conf']['member']['sessionTime'], '<font color="#FF0000">在线</font>', '离线') ?></td>

			<td style="width:10%"><?= date('Y-m-d', $var['regTime']) ?></td>
            <?php
        if ($this->user['uid'] != $var['uid'] && $var['parentId'] == $this->user['uid']) {
?>
			<td style="width:10%"> <a href="/index.php/team/userUpdate/<?= $var['uid'] ?>" style="color:#333;" target="modal"  width="420" title="修改用户" modal="true" button="确定:dataAddCode|取消:defaultCloseModal">修改</a>&nbsp;&nbsp;
			<?php
