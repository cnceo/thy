...
<head>
<?php
$this->display('inc_skin.php', 0, '已收消息 - 我的消息 - ');
?>
...
<form action="/index.php/box/searchReceive" datatype="html" call="boxSearch" target="ajax">
 	<h2>
     <select  style="width:80%" name="state" class="text5">
                <option value="3" selected>所有</option>
                <option value="2">已读</option>
                <option value="1">未读</option>
     </select>
            <input style="width:40%" type="text"  name="fromTime" class="datainput"  value="<?= $this->iff($_REQUEST['fromTime'], $_REQUEST['fromTime'], date('Y-m-d H:i:s', $GLOBALS['fromTime'])) ?>" id="datetimepicker" />至<input  style="width:40%"type="text" name="toTime"  class="datainput" value="<?= $this->iff($_REQUEST['toTime'], $_REQUEST['toTime'], date('Y-m-d H:i:s', $GLOBALS['toTime'])) ?>" id="datetimepicker4" /> 
