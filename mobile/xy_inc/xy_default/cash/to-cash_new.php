...
<?php
$this->display('inc_skin.php', 0, '会员中心 - 申请提现');
?>
...
<form action="/index.php/cash/ajaxToCash" method="post" target="ajax" datatype="json" onajax="beforeToCash" call="toCash">
  <div class="chongzhi3">
 	<h2>提款申请：</h2>
    <ul>
     <li><span>银行类型：</span><img src="/<?= $bank['logo'] ?>" title="<?= htmlspecialchars($bank['bankName']) ?>" alt="" /><a href="/index.php/safe/info#bank-info" > 修改信息>></a></li>
     ...
     <li><span>提款金额：</span><input type="text" name="amount" class="text4" id="ContentPlaceHolder1_txtMoney" onkeyup="showPaymentFee();" /><br/>*提现请输入<?= $this->settings['cashMin'] ?>至<?= $this->settings['cashMax'] ?>的整数金额！</li>
...
?>
 <div class="chongzhi2">
    ...
    <ul>
	 ...
	 <li>6、今日投注：<strong style="color:red"><?= $this->iff($cashAmout, $cashAmout, 0) ?></strong> 元&nbsp今天充值：&nbsp<strong style="color:red"><?= $this->iff($rechargeAmount, $rechargeAmount, 0) ?></strong>&nbsp元&nbsp，您今日消费比例已达到：&nbsp<strong style="color:red" id="bili"><?= $this->iff($rechargeAmount, round($cashAmout / $rechargeAmount * 100, 1), 100) ?>&nbsp%</strong>;</li>
    </ul>
 </div>
   		<?php
} else {
