...
if ($_REQUEST['fromTime'] && $_REQUEST['toTime']) {
    ...
    if ($GLOBALS['fromTime'] && $GLOBALS['toTime'])
        $timeWhere = ' and l.actionTime between ' . $GLOBALS['fromTime'] . ' and ' . $GLOBALS['toTime'] . ' ';
}
...
?>
		<tr class="even">
			<td><?php
        echo substr(date('Y-m-d H:i:s', $var['actionTime']), 2);
?></td>
...
?></td> -->
			...
			<?php
        if ($var['extfield0'] && in_array($var['liqType'], array(
            2,
            3,
            4,
            5,
...
?>
                <td><a href="/index.php/record/betInfo/<?= $var['extfield0'] ?>" width="100%" title="投注信息" target="modal"><?= $this->getValue("select wjorderId from {$this->prename}bets where id=?", $var['extfield0']) ?></a>
                </td>
               
			<?php
        } elseif (in_array($var['liqType'], array(
            1,
            9,
            52
        ))) {
?>
                <td><a href="/index.php/cash/rechargeModal/<?= $var['extfield0'] ?>" width="100%" title="充值信息" target="modal"><?= $var['extfield1'] ?></a></td>
                <td>--</td>
                <td>--</td>
                <td>--</td>
                <td>--</td>
...
?>
                <td><a href="/index.php/cash/cashModal/<?= $var['extfield0'] ?>" width="100%" title="提现信息" target="modal"><?= $var['extfield0'] ?></a></td>
                <td>--</td>
                <td>--</td>
                <td>--</td>
                <td>--</td>
